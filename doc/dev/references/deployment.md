...
A version `1.0.0` will be planned when features can demonstrate maturity by being
applied in different projects. Otherwise, there is no evidence of having developed the
right features and efforts on maintaining interface stability are wasted.
...
[pyproject.toml](gh) and
[cd.yml](gh:.github/workflows/cd.yml). It has three top level steps:
...
For linking the active version release page in docs, use
[gh:version](gh).
