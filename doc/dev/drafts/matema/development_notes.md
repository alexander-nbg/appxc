...
```{page-status} draft
:summary: page contents need to be structured and updated (2026/03)
```
...
* python script constructs objects and fills the test helper
...
The Script would contain the helper that launches the test case execution. This helper would display the case description, collect results and allow automated stimulation via buttons.
...
* (-) Strong requirement to the manual test case implementation: it has to provide the case results.
	* If case results are not available, the MaTeMa can only evaluate the process exit status.
...
* (+) Automation steps must still be provided by the script itself leading also to three windows as in "script is responsible". Only if this is not required, the MaTeMa can switch its view to the case execution, avoiding one of the three windows.
