...
from appxc import Options, Stateful


class AppxcSettingError(Exception):
    """AppxcSetting handling error"""


class AppxcSettingConversionError(Exception):
    """AppxcSetting conversion error
...
class _SettingMeta(type):
    ...
    """ Metaclass to trigger registration of new AppxcSetting classes """
...
class SettingOptions(Options):
    ...
    # feature (when utiliing the AppxcApplication frame and getting the config
...
class Setting(Stateful, Generic[_BaseTypeT], metaclass=_SettingMetaMerged):
    ...
    Use Setting.new('str') to get matching appxc settings for known types. You
    ...
    def __init__(self, value: _BaseTypeT | None = None, **kwargs):
        super().__init__(**kwargs)
    ...
    def _set_value(self, value: Any):
        ...
        if value == self.get_default():
            ...
            if not valid:
                raise AppxcSettingConversionError(type(self), value)
        self._input = value
        self._value = _value
