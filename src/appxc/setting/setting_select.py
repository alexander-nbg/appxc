...
from .setting import AppxcSettingError, Setting, _BaseTypeT
...
class SettingSelect(SettingExtension[_BaseSettingT, _BaseTypeT]):
    ...
    [sel = AppxcSetting.new('int', options={'one': 1, 'two': 2})].
    """

    # This AppxcSetting class is an extention on an existing AppxcSetting type
    # which is marked by this class attribute:
    setting_extension = "select"
    # The class can be instantiated either by AppxcSetting.new('int_select')
    # for string based type selection or via AppxcSetting[AppxcInt]() for
