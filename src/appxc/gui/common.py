...
class GridFrame(tk.LabelFrame):
    """Class to support general APPXC frames
    ...
    # Registry mapping APPXC object types to GridFrame subclasses
    ...
    @classmethod
    def get_frame(cls, parent: tk.BaseWidget, appxc_object, **kwargs):
...
class ButtonFrame(GridFrame):
    ...
    def set_button_active(self, button: str, active: bool):
        ...
        Raises AppxcGuiError if no such button exists.
        """
        widget = self._button_widgets.get(button)
        if not widget:
            raise AppxcGuiError(f"No button with name {button}")
