...
from appxc.storage import CompactSerializer, LocalStorage, Storage


class AppxcSecurityError(Exception):
    """General security related errors."""


class AppxcSecuritySignatureError(Exception):
...
class Security:
    ...
    def _verify_version(self):
        ...
        if "version" not in self._key_dict:
            raise AppxcSecurityError(
                "Not an APPXC security file: no version information",
            )
    ...
    def is_user_initialized(self):
        ...
        security.init_user(). Recommended is using an APPXC provided user
        interface.
        """
    ...
    def is_user_unlocked(self):
        ...
        Use Security.unlock_user() directly or an APPXC provided user interface.
        """
    ...
    def init_user(self, password):
        ...
        if self.is_user_initialized():
            raise AppxcSecurityError("Keys are already initialized.")
    ...
    def unlock_user(self, password):
        ...
        if not self.is_user_initialized():
            raise AppxcSecurityError("User is not initialized. Run init_user().")
        self._derived_key = self._derive_key(password)
        self._load_keys()
    ...
    def hybrid_signed_decrypt(
        ...
        Raises AppxcSecuritySignatureError if signature verification fails.
        ...
        if not Security.verify_signature(data, signature, author_pub_key):
            raise AppxcSecuritySignatureError
