...
from appxc.storage import CompactSerializer, LocalStorage, StorageToBytes, sync
...
class AppxcRegistryError(Exception):
    """General registry error"""


class AppxcRegistryUnknownUserError(Exception):
    """Unknown user"""


class AppxcRegistryUnitializedError(Exception):
...
# convention)
class AppxcRegistryUnknownConfigSectionError(Exception):
    """A referenced config section does not exist"""


class AppxcRegistryRoleError(Exception):
...
class Registry(RegistryBase):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if local_storage_factory is None:
            local_storage_factory = LocalStorage.get_factory(path="./data/security")
    ...
    # via the remote synchronization. Main question is: should APPXC support a
    ...
    def is_initialized(self) -> bool:
        # Documentation in RegistryBase
        return self._loaded or (self._user_id.exists() and self._user_db.exists())

    def _ensure_loaded(self):
        if self._loaded:
            return
        if not self.try_load():
            AppxcRegistryUnitializedError("Registry is not initialized.")
    ...
    def verify_signature(
        ...
        if roles:
            ...
            if not has_role:
                self.log.warning(
                    "Signing user %i does not have expected roles %sthey only have %s",
                    signing_user,
                    str(roles),
                    str(self.get_roles(signing_user)),
                )
                return False
    ...
    def set_admin_key_bytes(self, data: bytes):
        ...
        if self.is_initialized():
            raise AppxcRegistryError("Cannot set admin keys on initialized registry.")
        ...
        for key_tuple in key_list:
            self._user_db.add(key_tuple[0], key_tuple[1], key_tuple[2], roles=["admin"])
        self._user_db.store()
    ...
    def get_request_bytes(self) -> bytes:
        ...
        request = {"request": bytes_encrypted, "key_blob_dict": key_blob_dict}
    ...
    def add_user_from_request(
        ...
        if roles is None:
            roles = ["user"]
        if not self._loaded:
            raise AppxcRegistryUnitializedError(
    ...
    def get_response_bytes(
        ...
        for section in self._response_config_sections:
            if section not in self._config.sections:
                raise AppxcRegistryUnknownConfigSectionError(
    ...
    def set_response_bytes(self, response_bytes: bytes):
        ...
        if not self.verify_signature(
            ...
            raise AppxcRegistryError("Signature could not be verified")
        ...
        for section in response.config_sections:
            self._config.section(section).update(response.config_sections[section])
        ...
        self.log.info(
            "Registration response assigned USER ID %i and included config section %s.",
            response.user_id,
            str(response.config_sections.keys()),
        )
    ...
    def hybrid_encrypt(
        ...
        if self.is_initialized() and self.user_id not in pub_key_dict:
            pub_key_dict[self.user_id] = self._security.get_encryption_public_key()
    ...
    def set_manual_config_update_bytes(self, data: bytes):
        ...
        if author_id is None:
            raise AppxcRegistryUnknownUserError(
    ...
    def sync_with_remote(self, mode: str):
        ...
        if self.try_load():
            ...
            if mode == "receiving":
                ...
                sync(
                    storage_a=self._remote_user_db_storage,
                ...
                if is_admin:
                    print(" -- sending")
                    sync(
                        storage_a=self._local_user_db_storage,
                        storage_b=self._remote_user_db_storage,
                        only_a_to_b=True,
                    )
            else:
                raise AppxcRegistryError(f"Mode {mode} is unknown.")
